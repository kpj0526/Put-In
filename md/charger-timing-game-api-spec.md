...
- `POST /api/auth/register`: 회원가입 후 인증 메일을 발송한다.
- `POST /api/auth/verify-email`: 인증 토큰으로 이메일 인증을 완료하고 access token을 발급한다.
- `POST /api/auth/resend-verification`: 인증 메일을 재발송한다.
- 이메일 인증 전 로그인 시 `EMAIL_NOT_VERIFIED` 에러를 반환한다.
- 이메일 인증 토큰은 원문을 DB에 저장하지 않고 해시만 저장한다.
